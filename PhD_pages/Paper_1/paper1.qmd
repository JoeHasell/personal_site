...
# READING LIST:
# Bourguignon (2015)
# WID methodology papers:

# - Brazil: https://wid.world/document/extreme-persistent-inequality-new-evidence-brazil-combining-national-accounts-surveys-fiscal-data-2001-2015-wid-world-working-paper-201712/
#  - Assouad (2017) on Lebanon and Czajka (2017) on Côte d’Ivoire]
...
This disconnect is particularly problematic given the starkly contrasting perspectives on inequality found in studies drawing on the two datasets.
...
Alongside these divergent assesments of *how* inequality has changed around the world, the two approaches to measurement are frequently used to evidence contrasting accounts of *why* it has changed in this way, and hence where to target efforts to combat inequality in the future. [TODO: Expand this thought: i.e. marginal labour productivies vs capital incomes/top wages bargaining & top marginal tax rates. National vs global factors, policy vs more fundamental forces.].

In sum, the two 'worlds' of inequality measurement adopted by the *PIP* and *WID* datasets have yielded two very different views of inequality around the world and consequently two very different sets of recommendations as to how to address it.

Which of these realities should we give most credence to? A simplistic reading of the top incomes literature would see the *WID* data as unambiguously providing greater reliability, thanks to the incorporation of tax data and other sources in order to correct for the well-known issue of the 'missing rich' within survey data [@lustig_missing_2019]. The case for such a correction is amply demonstrated in many instances. @korinek_survey_2006, for example, find richer households in the United States are less likely to participate in surveys, and they estimate that this biases the Gini coefficient downwards by around five percentage points. [TODO: More examples, including underreporting]. The *PIP* data, based purely on survey data, makes no such correction and is thus likely to understimate the extent of inequality in many cases.

A more agnostic assessment of the two datasets is, however, warrented on a number of grounds. Firstly, the extent of the upward revision to inequality estimates resulting from top income adjustments varies considerably from country to country, and in some cases may be quite modest. This is demonstrated by @carranza_top_2021 who apply a *WID*-style top income adjustment to survey data for 26 European countries and find an average increase in the Gini of around 2.4 points, with notable differences across countries and limited impact on trends. Secondly, for many countries/periods included in the *WID* estimates, the richer range of data sources that the dataset's methodology seeks to incorporate do not exist. In the place of data, various imputation and interpolation strategies must be employed.^[For example, see the methods outlined in @chancel_towards_2019 in the context of Africa, for which data quality and coverage is especially low] The terms of this tradeoff – between drawing on a greater set of information, and needing to fill in the gaps where this information does not exist – are not clear. While the incorporation of tax data is a proven asset in many individual cases, the value and reliability of top income adjustment methods when applied globally is harder to demonstrate. Lastly, deviations between survey and tax data appear to be restricted to the *very* top of the distribution. After aligning the income concept across survey and tax data for United States, Germany and France, @yonzan_drawing_2022 find that the sources correspond extremely well up until the top 1% of the distribution. The apparent 'accuracy' of survey data is thus likely to depend on the inequality index chosen, and in particular to its sensitity to changes at the top of the distribution. It is thus a question whose answer will depend also on normative judgements concerning the nature of inequality and how it should be measured [@atkinson_measurement_1970].  

Overall, while much can be said about the relative strengths and weaknesses of the two approaches to measurement employed in the datasets for particular questions and cases, there is no straightforward answer to the question of which offers us the most reliable or relevant *global* view on inequality.  

A different route, however, may be taken to try to reconcile the divergent views of inequality often drawn from the two datasets. A substantial limitation of existing work on inequality around the world is that consideration of the *PIP* and *WID* data tends to be given in insolation.[^isolated_examples] As a consequence, areas of *agreement* between the dataset are overlooked, and the reasons behind disagreements – of which, given the range of methodological differences, there may be many – are given little consideration. By using the two datasets in conjunction, rather than in isolation, we can 'triangulate' a more robust and coherent empirical view informed by both. [TODO: Add footnote citing Tony Atkinson on the idea of triangulation]. This is the first of a pair of papers aiming to do this. The present paper focuses on trends in within-country inequality and a companion paper takes a similar approach but considers overall global inequality.

[^isolated_examples]:
    TODO: finish footnote.
    Examples include the flagship reports associated with the two datasets – see for example @chancel_global_2021 and @world_bank_poverty_2022. Policy papers: @undp_beyond_2019 consider only WID data, with some minor cross-references to PIP data. Within academic research @kanbur_global_2022, @nino-zarazua_global_2017 and @ravallion_income_2014 rely on PIP data. Explain about @van_zanden_changing_2014.

    Two notable counter-examples that have sought to incorporate both 'worlds' are @anand_global_2015 and @lakner_global_2016. Both studies recalculate global inequality measures after making a rough adjustment to the survey data so as to match the top income shares observed in *WTID*, a successor to *WID*. Their estimates extend only to 2005 and 2008 respectively and were made prior to the substantial increases in coverage now available in *WID*. Moreover, neither consider average within-country trends – the primary focus of the present paper.

    @united_nations_world_2020 also draws on evidence from both datasets, but presents the results in parallel, without attempting to draw from them a coherent view.

I triangulate between the *PIP* and *WID* data by separately aligning coverage and the inequality indices used. The result of this is X, Y, Z. For a subset of countries, the welfare concept, using Luxembourg Income Study, as well as *WID*'s own estiamtes of post-tax inequality where available.
...
[TODO: More details on PIP and WID and how they differ in terms of concepts and methods.

- Include discussion of inter/extrapolation in WID data. E.g. Rescaling factors from tax data for one year is often applied to many other years. In the case of Africa, the rescalling factors seen in the few countries with tax data (for at least some years) are applied to all other countries.]

- Include discussion of comparability and quality of survey data in PIP.

- Plot forthe appendix: make a direct comparison for Africa region with the same inequality index. Necessarily tethered to the survey data (unlike say Mexico) The changes are amplified in WID (falls are larger, rises are larger).
...
# 3. An out-of-the-box comparison of the *PIP* and *WID* datasets

[TODO: Describe the basic method used to compare the data:

- I take pairs of estimates in or close to reference years
    - Footnote: For PIP data, which is a mix of income and consumption points, I only take a pair of observations where the income concept does not match where matchin observations are not available. This is only the case for 3 out of 90 countries in one specification.
    - I use two periods, chosen to hit two different points in the trade-off maximising coverage and the length/relevance of the period studied: 1993-2015 (+/- 3 years) and 1980-2018 (+/- 5 years)

- I consider unweighted and population-weighted average trends, and also the number of countries seeing substantive rises or falls.

]

We begin by comparing trends in the inequality metrics most characteristic of the two 'worlds' of measurement and where the datasets fall the furthest apart conceptually: for *PIP* the Gini of disposabale income (or consumption); for *WID* the top 1% share of pretax national income.
...
output_dict_1993 = prep_data_and_change_summary(
...
df_owid_1993_2015 = output_dict_1993['owid_format']
...
@fig-pip-wid-1993-2015-all-obs compares the level of inequality, as measured by the two metrics, in or around 1993 and 2015. A band is shown along the 45-degree line. Countries falling within this band saw little change in inequality. For countries above the band, inequality in 2015 was substantively higher than in 1993. Countries falling below the band instead saw a substantive fall. The data is summarised in @tbl-pip-wid-1993-2015-all-obs-absolute-count, which provides a counts of the number of countries with rising, stable or falling inequality, and @tbl-pip-wid-1993-2015-all-obs-averages which presents unweighted and population-weighted averages for the two reference years. In both @fig-pip-wid-1993-2015-all-obs and @tbl-pip-wid-1993-2015-all-obs-absolute-count, a 'substantive change over the period is defined as a movement of more than 2 percentage points in the case of the PIP Gini coefficient, and more than 1 percentage point in the case of the WID top 1% share. [TODO: add table in appendix showing that using *relative* thresholds – i.e. +/-5% (and not %-point) across both metrics – makes little difference.]
...
output_dict_1993['all_sources_False']['absolute_counts']
...
output_dict_1993['all_sources_False']['averages']

```


According to *PIP* estimates for the Gini coefficient, inequality was rising and falling in roughly an equal number of countries over the period. This broadly replicates the analysis presented in @world_bank_poverty_2016. The unweighted average fell somewhat over the period, from 39.8 to 30.0 points. As we see from @fig-pip-wid-1993-2015-all-obs (top panel), contributing to this average decline are a number of countries showing very substantial falls – in some cases declines of 10 points or more. These are seen mostly in Sub-Saharan Africa, in addition to a number of Latin American and Eastern European and Central Asian countries. [TODO: Mention particular questions around survey comparability in SSA.]
...
As @fig-pip-vs-wid-1993-2015 (section 4) illustrates, it is also in particularly populous countries where we see some of the starkest differences between the two datasets. The figure plots the change in the two metrics, along with the line of best fit. 
[TODO: 
...
```{python}
# Set-up to compare PIP Gini and WID Top 1% share, all observations, for longer period
...
output_dict_1980 = prep_data_and_change_summary(
      source_dfs = source_dfs,
      df_pop = df_pop,
      df_pop_regions = df_pop_regions,
      df_regions = df_regions,
...
# Store data for OWID upload – prepared further in appendix below
df_owid_1980_2018 = output_dict_1980['owid_format']
...
[TODO: Make similar figure as figure 1]


```{python}
#| label: tbl-pip-wid-1980-2018-all-obs-absolute-count
#| tbl-cap: Change in inequality 1980 vs 2018 - All availalbe observations
output_dict_1980['all_sources_False']['absolute_counts']
...
output_dict_1980['all_sources_False']['averages']
...
[TODO: Plot in appendix – Just like figure 1, but show only the data points missing from WID in the PIP pannel. And visa versa.]

```{python}
import statsmodels.formula.api as smf
from matplotlib import pyplot as plt

change_compare = output_dict_1993['all_sources_True']['prepped_data'][['source_var', 'Entity', 'change', 'region']]
...
[TODO: same plot but for 1980 period]
...
It sometimes goes the other way: 
- Indonesia: Rise in PIP, fall in WID.
- Some SSA countries: Larger falls in WID.
...
- For a range of metrics, check which countries disagree on change category using a relative definitnio (+/-5%). And report global averages.


```{python}
# Set-up to compare PIP
reference_vals = [1993, 2015]
max_dist_from_refs = 3
...
output_dict_PIP_indices_1993 = prep_data_and_change_summary(
      source_dfs = source_dfs,
      df_pop = df_pop,
      df_pop_regions = df_pop_regions,
      df_regions = df_regions,
...
```{python}
#| label: tbl-pip-indices-counts
#| tbl-cap: Change in inequality 1980 vs 2018 - All availalbe observations
output_dict_PIP_indices_1993['all_sources_False']['absolute_counts']
...
```{python}
#| label: tbl-pip-indices-avgs
#| tbl-cap: Unweighted and weighted average in 1980 vs 2018 - All availalbe observations
output_dict_PIP_indices_1993['all_sources_False']['averages']
...
# 5. Bridging the gap for particular countries / welfare concept
...
## Comparison of the WID and PIP datasets after aligning coverage

### Inequality since 1993


#### Figure X
![PIP Gini and WID Top 1% share in 1993 vs 2015 – Matching observations only](images/figures/PIPGini-vs-WIDTop1-1993-vs-2015-02.png){#fig-pip-wid-1993-2015-matching-obs}
...
output_dict_1993['all_sources_True']['absolute_counts']
...
output_dict_1993['all_sources_True']['averages']

```

### Inequality since 1980

[TODO: same as above for post-1993 period.]
...
output_dict_1980['all_sources_True']['averages']
